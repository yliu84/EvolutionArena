# Valley passive grazer / 阶地岩羚 source record

Status: **Source supplied and processed on 2026-08-18.** Do not start Blender cleanup, retopology or rigging until the downloaded mesh passes the silhouette gates below.

- Prepared: 2026-08-18
- Identity: 河谷被动食草怪 / 阶地岩羚 (`terrace-grazer`)
...
- Downstream runtime (not this step): `public/assets/quality-3d/models/terrace-grazer-rigged-v1.glb`

When Meshy finishes, drop the downloaded textured GLB into **this folder** (`docs/concepts/valley/terrace-grazer/source/`). Keep the original Meshy filename, then record it, the SHA-256 and the selected license in the blank fields below. The source GLB is immutable. Derived cleanup/rig files must use separately named paths.
...
| Order | Role | File |
| --- | --- | --- |
| 1 | **Primary reconstruction image** (3/4 standing pose). Use this alone if Meshy is in single-image mode. | `docs/EvolutionArena-Project-Docs-v0.1/docs/art/concepts/valley-terrace-grazer-meshy-source-three-quarter.png` |
| 2 | Extra view — right profile, length vs height, snout, dorsal scutes, tail | `docs/EvolutionArena-Project-Docs-v0.1/docs/art/concepts/valley-terrace-grazer-meshy-source-side-right.png` |
| 3 | Extra view — left profile, exact horizontal mirror of view 2 | `docs/EvolutionArena-Project-Docs-v0.1/docs/art/concepts/valley-terrace-grazer-meshy-source-side-left.png` |
...
- `valley-terrace-grazer-concept.png` as a second primary (it is the same pose as the three-quarter source; use the `meshy-source-three-quarter` filename)
