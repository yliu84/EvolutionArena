...
| # | Date | Method | Outcome |
| --- | --- | --- | --- |
| 3 | 2026-08-17 | Meshy image-to-3D, side view added as a second reconstruction slot | **Accepted, current runtime.** 20,391 triangles, 27 bones, names matching the runtime contract. Length:height came out **2.54** against attempt 2's 3.23 — the 3/4 primary view was foreshortening the body and Meshy was guessing it long, which the true side view corrects. At the 1.80 form height that is 1.58 wide x 4.57 long, against the Fang form's 1.56 x 3.99, so the footprint concern from attempt 2 is largely resolved. A small static gap remains between two plates near the top centre of the shell; it is present in the rest pose, so it is a mesh seam rather than a rigging or animation fault. Runtime file `stone-pangolin-rigged-runtime-v2.glb`, 8,197,152 -> 1,737,992 bytes, validator clean. |
