...
import { applySpeciesMechanics, combatStyleForSpecies, quality3DAssetStageForSpecies, resolveAcceptanceSpecies, resolveEvolutionSpecies, speciesDebugContract } from './evolution-species'
...
class PrototypeScene extends Phaser.Scene {
  ...
  private apexSpeciesId: string | null = null
  ...
  create() {
    ...
    this.boss.setCircle(54).setData('hurtRadius', 68).disableBody(true, true)
  ...
  getDebugState() {
    ...
    return {
      ...
      evolutionSystem: {
        ...
        formName: resolveEvolutionSpecies(this.evolutionStage, this.genes, this.recentHunts, this.mutationRanks, this.evolutionChain, this.apexSpeciesId).formName,
        ...
        appearance: (() => {
          const species = resolveEvolutionSpecies(this.evolutionStage, this.genes, this.recentHunts, this.mutationRanks, this.evolutionChain, this.apexSpeciesId)
          const appearance = playerEvolutionAppearance(this.evolutionStage, species.primaryFamily, this.mutationRanks, species.secondaryFamily)
          return {
            ...appearance,
        ...
        species: speciesDebugContract(resolveEvolutionSpecies(this.evolutionStage, this.genes, this.recentHunts, this.mutationRanks, this.evolutionChain, this.apexSpeciesId)),
  ...
  applyDebugEvolutionRoute(family: GeneFamily, stage: number) {
    ...
    if (this.evolutionStage === EVOLUTION_CONFIG.maxStages) {
      const species = resolveEvolutionSpecies(this.evolutionStage, this.genes, this.recentHunts, this.mutationRanks)
      this.apexSpeciesId = species.definition.id
      this.combatStyle = combatStyleForSpecies(species.definition)
    }
  ...
  private renderHud() {
    ...
    const formName = resolveEvolutionSpecies(this.evolutionStage, this.genes, this.recentHunts, this.mutationRanks, this.evolutionChain, this.apexSpeciesId).formName
  ...
  private attackSelectedOrPoint(fallbackX: number, fallbackY: number, allowBuffer = true) {
    ...
    if (this.selectedTarget) {
      ...
      if (!isWithinAttackRange(this.combatStyle, targetDistance, this.targetHurtRadius(this.selectedTarget))) {
  ...
  private executeMeleeAttack(attack: PendingPlayerAttack) {
    ...
    for (const target of this.getDamageableTargets()) {
      if (!isInsideMeleeArc(
        ...
        target.x,
        target.y,
        COMBAT_STYLES.melee.range,
        Math.PI * 0.36,
        this.targetHurtRadius(target),
      )) continue
  ...
  private commitHuntEvolution() {
    ...
    const mutationStats = applyMutationEffect(this.mutationStats(), resolved.mutation.effect)
    this.evolutionStage += 1
    this.evolution = 0
    ...
    const species = resolveEvolutionSpecies(this.evolutionStage, this.genes, this.recentHunts, this.mutationRanks, this.evolutionChain, this.apexSpeciesId)
    if (this.evolutionStage === EVOLUTION_CONFIG.maxStages) this.apexSpeciesId = species.definition.id
    this.combatStyle = combatStyleForSpecies(species.definition)
    this.writeMutationStats(this.evolutionStage === EVOLUTION_CONFIG.maxStages
      ? applySpeciesMechanics(mutationStats, species.definition)
      : mutationStats)
    this.invulnerableUntil = this.time.now + 900
    this.evolutionBurstUntil = this.time.now + 1100
    this.syncPlayerBodyScale()
    const form = species.formName
  ...
  private renderPlayerEvolutionAppearance(time: number) {
    ...
    const species = resolveEvolutionSpecies(this.evolutionStage, this.genes, this.recentHunts, ranks, this.evolutionChain, this.apexSpeciesId)
    const dominant = species.primaryFamily
    const appearance = playerEvolutionAppearance(this.evolutionStage, dominant, ranks, species.secondaryFamily)
...
function launchGloamwoodExplorationLab() {
  ...
  const motherMonsterRequested = liveRunRequested && searchParams.get('mother') === '1'
  const evolutionStageRequested = Number(searchParams.get('evolutionStage'))
  const evolutionRouteValue = searchParams.get('evolutionRoute')
  const evolutionSecondaryValue = searchParams.get('evolutionSecondary')
  const evolutionRouteRequested = GENE_FAMILIES.includes(evolutionRouteValue as GeneFamily) ? evolutionRouteValue as GeneFamily : null
  const evolutionSecondaryRequested = GENE_FAMILIES.includes(evolutionSecondaryValue as GeneFamily) ? evolutionSecondaryValue as GeneFamily : undefined
  const acceptanceSpecies = evolutionRouteRequested && evolutionStageRequested > 0
    ? resolveAcceptanceSpecies(evolutionStageRequested, evolutionRouteRequested, evolutionSecondaryRequested)
    : null
  const motherMonsterActive = motherMonsterRequested
    && (!acceptanceSpecies || quality3DAssetStageForSpecies(acceptanceSpecies, evolutionStageRequested) !== null)
  const scarletGeckoRequested = motherMonsterRequested && evolutionStageRequested >= 1
  const scarletHunterRequested = motherMonsterRequested && evolutionStageRequested >= 2
  ...
  document.querySelector<HTMLElement>('#bestiary-overlay')?.setAttribute('hidden', '')
  const badge = document.querySelector<HTMLElement>('.prototype-badge')
  if (badge) badge.textContent = acceptanceSpecies
      ? `幽影林地 · ${acceptanceSpecies.formName} · Random Evolution`
      : motherMonsterActive
        ? scarletHunterRequested
        ? '幽影林地 · Scarlet Hunter Evolution'
        : scarletGeckoRequested
          ? '幽影林地 · Scarlet Gecko Evolution'
          : '幽影林地 · Mother Monster Hunt'
    : liveRunRequested ? '幽影林地 · Hunt & Evolve' : 'Map Lab V4 · 八大特色战斗窝点'
  const help = document.querySelector<HTMLElement>('.helpbar')
  if (help) {
    help.innerHTML = [
      ...
      debugRequested && liveRunRequested ? '<button class="maplab-stage-button" type="button" data-exploration-action="mother-target"><strong>Tab</strong> QA锁定</button>' : '',
      debugRequested && liveRunRequested ? '<button class="maplab-stage-button" type="button" data-exploration-action="mother-attack"><strong>Space</strong> QA普攻</button>' : '',
      ...
      `<span class="maplab-camera-help"><strong>WASD</strong> 移动 · <strong>点击/Tab</strong> 锁定 · ${motherMonsterActive ? scarletHunterRequested ? '<strong>Space</strong> 扑爪 → 重咬 → 尾砸' : '<strong>Space</strong> 咬击 → 爪击 → 尾扫' : '<strong>1/2/3 + Space</strong> 普通攻击'}${liveRunRequested ? ' · <strong>R</strong> 抗拒生长' : ''}</span>`,
      '<span class="divider">/</span>',
      `<span class="optional">${acceptanceSpecies ? `第${evolutionStageRequested}次进化 ${acceptanceSpecies.formName} · ${acceptanceSpecies.definition.normalAttackProfile} · 技能系统保持关闭` : motherMonsterActive ? scarletHunterRequested ? '第二次进化赤爪猎龙 · 技能系统保持关闭' : scarletGeckoRequested ? '第一次进化赤冠壁蜥 · 技能系统保持关闭' : '母怪物GLB正式猎杀纵切片 · 技能系统保持关闭' : liveRunRequested ? '猎杀吞噬决定进化 · 清理窝点后挑战古林之心' : '8个窝点拥有独立入口、2/3波节奏、机制、核心与奖励'}</span>`,
    ].join('')
  }
  ...
  let formalHud: HTMLElement | null = null
  let pauseOverlay: HTMLElement | null = null
  if (liveRunRequested) {
    formalHud = document.createElement('section')
    formalHud.className = 'formal-hunt-hud'
    formalHud.dataset.formalHuntHud = ''
    formalHud.setAttribute('aria-label', '正式猎杀状态')
    formalHud.innerHTML = [
      '<div class="formal-hunt-objective">',
      '  <span class="formal-hunt-kicker">当前猎迹 / ACTIVE HUNT</span>',
      '  <strong data-hud-objective>探索幽影林地</strong>',
      '  <span class="formal-hunt-compass" data-hud-compass aria-hidden="true"><i></i></span>',
      '  <span data-hud-distance>--m</span>',
      '</div>',
      '<section class="formal-hunt-vitals" aria-label="生命与形态">',
      '  <header><span data-hud-form>原始形态</span><small data-hud-stage>进化 0/6</small></header>',
      '  <strong data-hud-species>珊瑚冠壁蜥</strong>',
      '  <div class="formal-hunt-meter health" data-hud-health-bar><i></i><span data-hud-health>90 / 90</span></div>',
      '  <div class="formal-hunt-meter evolution" data-hud-evolution-bar><i></i><span data-hud-evolution>0/20</span></div>',
      '  <p data-hud-gene>尚未形成倾向</p>',
      '</section>',
      '<section class="formal-hunt-route" aria-label="随机进化路线">',
      '  <span class="formal-hunt-kicker">随机进化 / EVOLUTION ROUTE</span>',
      '  <strong data-hud-gate>终局封印</strong>',
      '  <p data-hud-resist>随机进化由猎物决定</p>',
      '  <button type="button" data-formal-action="resist">抗拒生长 · R</button>',
      '</section>',
      '<section class="formal-hunt-target" aria-live="polite">',
      '  <strong data-hud-target>点击怪物或按 Tab 锁定目标</strong>',
      '  <span data-hud-event>沿猎路寻找第一个窝点</span>',
      '</section>',
      '<section class="formal-hunt-actions" aria-label="猎杀操作">',
      '  <button type="button" data-formal-action="target"><span>锁定</span><b>Tab</b></button>',
      '  <button class="primary" type="button" data-formal-action="attack"><span>普通攻击</span><b data-hud-attack>咬击 → 爪击 → 尾扫</b></button>',
      '  <button type="button" data-formal-action="pause" aria-label="暂停游戏"><span>暂停</span><b>Esc</b></button>',
      '</section>',
      '<section class="formal-hunt-boss" data-hud-boss hidden>',
      '  <header><strong data-hud-boss-name>裂隙守望者</strong><span data-hud-boss-health>0/0</span></header>',
      '  <div class="formal-hunt-meter boss" data-hud-boss-bar><i></i></div>',
      '</section>',
    ].join('')
    pauseOverlay = document.createElement('section')
    pauseOverlay.className = 'formal-hunt-pause'
    pauseOverlay.dataset.formalPause = ''
    pauseOverlay.hidden = true
    pauseOverlay.setAttribute('aria-labelledby', 'formal-pause-title')
    pauseOverlay.innerHTML = [
      '<div>',
      '  <span class="formal-hunt-kicker">猎杀暂停 / HUNT PAUSED</span>',
      '  <h2 id="formal-pause-title">古林仍在呼吸</h2>',
      '  <p>本局随机进化路线与猎杀进度会保留到继续游戏。</p>',
      '  <button type="button" data-formal-action="resume">继续猎杀</button>',
      '  <button class="secondary" type="button" data-formal-action="restart">重新开始本局</button>',
      '</div>',
    ].join('')
    document.querySelector('#game-container')?.append(formalHud, pauseOverlay)
  }
  game = new Phaser.Game({
    type: Phaser.AUTO,
    parent: 'game-container',
  ...
  const setFormalPaused = (paused: boolean) => {
    if (!pauseOverlay || !game) return
    pauseOverlay.hidden = !paused
    pauseOverlay.classList.toggle('is-open', paused)
    if (paused) {
      game.scene.pause(GLOAMWOOD_EXPLORATION_LAYOUT.sceneKey)
      window.setTimeout(() => pauseOverlay?.querySelector<HTMLButtonElement>('[data-formal-action="resume"]')?.focus(), 30)
    } else {
      game.scene.resume(GLOAMWOOD_EXPLORATION_LAYOUT.sceneKey)
      document.querySelector<HTMLCanvasElement>('#game-container canvas')?.focus()
    }
  }
  formalHud?.querySelector<HTMLButtonElement>('[data-formal-action="attack"]')?.addEventListener('click', () => scene().requestMotherMonsterBasicAttack())
  formalHud?.querySelector<HTMLButtonElement>('[data-formal-action="target"]')?.addEventListener('click', () => scene().selectNextMotherMonsterTarget())
  formalHud?.querySelector<HTMLButtonElement>('[data-formal-action="resist"]')?.addEventListener('click', () => scene().resistFormalEvolution())
  formalHud?.querySelector<HTMLButtonElement>('[data-formal-action="pause"]')?.addEventListener('click', () => setFormalPaused(true))
  pauseOverlay?.querySelector<HTMLButtonElement>('[data-formal-action="resume"]')?.addEventListener('click', () => setFormalPaused(false))
  pauseOverlay?.querySelector<HTMLButtonElement>('[data-formal-action="restart"]')?.addEventListener('click', () => window.location.reload())
  if (import.meta.env.DEV || debugRequested) {
    const debugApi = {
      getState: () => scene().getDebugState(),
    ...
    if (debugRequested) {
      ...
      window.setInterval(() => {
        try {
          ...
          // Phaser and the larger GLB can initialize on different frames; keep
          // polling so a transient startup miss does not disable QA telemetry.
        }
      }, 250)
    }
  }
  ...
  document.querySelector<HTMLButtonElement>('[data-exploration-action="mother-target"]')?.addEventListener('click', () => scene().selectNextMotherMonsterTarget())
  document.querySelector<HTMLButtonElement>('[data-exploration-action="mother-attack"]')?.addEventListener('click', () => scene().requestMotherMonsterBasicAttack())
  ...
  window.addEventListener('keydown', (event) => {
    if (event.key === 'Escape' && liveRunRequested && pauseOverlay) {
      event.preventDefault()
      setFormalPaused(pauseOverlay.hidden !== false)
      return
    }
